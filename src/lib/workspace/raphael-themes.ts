export type RaphaelThemeId = "apple";

export type RaphaelTheme = {
  id: RaphaelThemeId;
  name: string;
  description: string;
...
export const RAPHAEL_DEFAULT_THEME_ID: RaphaelThemeId = "apple";

export const RAPHAEL_THEMES: RaphaelTheme[] = [
  {
    id: "apple",
    name: "Mac",
...
export function resolveRaphaelTheme(themeId?: string) {
  return (
    RAPHAEL_THEMES.find((theme) => theme.id === themeId) ??
    RAPHAEL_THEMES.find((theme) => theme.id === RAPHAEL_DEFAULT_THEME_ID) ??
    RAPHAEL_THEMES[0]
  );
}
