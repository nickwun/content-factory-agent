...
const SEEDED_RETRY_RECORD = {
  id: "seeded-xiaohongshu-retry-record",
  schemaVersion: 1,
...
async function main() {
  ...
  const report = {
    pageLoaded: false,
    retryFlow: [],
    imageGeneratedAfterRetry: false,
    errorCleared: false,
  ...
  try {
    ...
    const firstSuggestion =
      historyPayload[0]?.content?.xiaohongshu?.imageSuggestions?.[0] ?? null;

    report.errorCleared = !firstSuggestion?.imageError;
